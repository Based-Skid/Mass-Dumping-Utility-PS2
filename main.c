...
void LoadModules(void)
{
	...
	lm = SifExecModuleBuffer(&iomanX, size_iomanX, 0, NULL, NULL);
	if (lm < 0) 
	{
	...
	lm = SifExecModuleBuffer(&fileXio, size_fileXio, 0, NULL, NULL);
	if (lm < 0) 
	{
	...
	lm = SifExecModuleBuffer(&MCTOOLS_irx, size_MCTOOLS_irx, 0, NULL, NULL);
	if (lm < 0) 
	{
		printf("Could not load MCTOOLS Module");
		gotoOSDSYS(6);
	}
	
	lm = SifExecModuleBuffer(&USBD, size_USBD, 0, NULL, NULL);
	if (lm < 0) 
	{
		printf("Failed to Load USBD Module");
		gotoOSDSYS(9);
	}
	
	lm = SifExecModuleBuffer(&USBHDFSD, size_USBHDFSD, 0, NULL, NULL);
	if (lm < 0) 
	{
